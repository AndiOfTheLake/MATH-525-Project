...
Recall from lecture that a greater value of $SSB-\sum_{h=1}^H S^2_h=\sum_{h=1}^H N_h (\bar{y}_{hu}-\bar{y}_u)^2-\sum_{h=1}^H S^2_h$ leads to a more efficient proportional allocation. Therefore, we only have to check which variable produces the larger $SSB$, and this variable is the more efficient stratification variable. Of course, we cannot compute $SSB$ since it (indirectly) involves the parameter of interest. Therefore, we make the comparison by using an estimate of $SSE:$

$$
\widehat{SSB}=\sum_{h=1}^H N_h (\hat{\bar{y}}_{hu}-\hat{\bar{y}}_u)^2,
$$

where $\hat{\bar{y}}_{hu}$ and $\hat{\bar{y}}_u$ are sample means.
...
Hence $\widehat{SSB}$ based on ``continent" variable is 621.
...
Hence $\widehat{SSB}$ based on ``Branch" variable is 1410.  

Since $\widehat{SSB}$ based on ``Branch" variable is greater, we should choose ``Branch" as stratification variable.
